...
def _admin_afa_settings_price(default: float = 0.0) -> float:
    try:
        doc = afa_settings_col.find_one({"_id": "AFA_SETTINGS"}, {"price": 1})
...
def _afa_setup_issue_for_store(store_doc: Dict[str, Any], base_price: float) -> Optional[Dict[str, Any]]:
    cfg = (store_doc or {}).get("afa_product") or {}
    if not isinstance(cfg, dict) or not cfg.get("enabled") or base_price <= 0:
        return None
    price = _to_float(cfg.get("price")) or 0.0
    if price >= base_price:
        return None
    slug = (store_doc.get("slug") or "").strip()
    return {
        "slug": slug,
        "store_name": (store_doc.get("name") or slug or "your store").strip(),
        "price": round(float(price), 2),
        "base_price": round(float(base_price), 2),
        "edit_url": "/create-store?slug=" + slug if slug else "/create-store",
    }
...
def _customer_price_for_offer(
    ...
    manual = _to_float(offer.get("customer_price"))
    if manual is not None:
        return round(manual, 2)
    return _to_float(offer.get("amount"))
...
def api_store_notifications():
    ...
    try:
        ...
        for od in cur:
            ...
            is_ussd_order = str(od.get("paid_from") or "").strip().lower() == "ussd" or bool(od.get("ussd"))
            orders_view.append({
...
def customer_dashboard():
    ...
    # services (sorted)
    raw_services = list(services_col.find(_visible_services_query()))
    ...
    # Agent code for the logged-in customer account
    agent_code_doc = agent_codes_col.find_one(
        {
            "$or": [
                {"user_id": user_oid},
    ...
    agent_code_status = ((agent_code_doc or {}).get("status") or "").strip().lower()
    ...
    afa_setup_issue: Optional[Dict[str, Any]] = None
    try:
        owned_stores = list(
            stores_col.find(
                {"owner_id": user_oid, "status": {"$ne": "deleted"}},
                {"slug": 1, "name": 1, "afa_product": 1}
            )
        )
        store_slugs = [s.get("slug") for s in owned_stores if s.get("slug")]
        afa_base_price = _admin_afa_settings_price(default=0.0)
        for store_doc in owned_stores:
            afa_setup_issue = _afa_setup_issue_for_store(store_doc, afa_base_price)
            if afa_setup_issue:
                break
        if store_slugs:
            pipeline = [
    ...
    return render_template(
        ...
        agent_code_status=agent_code_status,
        ...
        afa_setup_issue=afa_setup_issue,
        phone_history_verification_enabled=_phone_history_verification_enabled(),
        announcement_popup=get_popup_announcement("customer_dashboard", user_oid),
        announcement_location="customer_dashboard",
    )
