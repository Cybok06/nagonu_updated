...
def _provider_label(provider: str | None) -> str:
    ...
    if not prov:
        prov = "datakazina"
    return SERVICE_PROVIDER_LABELS.get(prov, prov)
...
def _service_display_on(service_doc: dict | None) -> bool:
    if not service_doc:
        return True
    return service_doc.get("display_on") is not False
...
def _is_mtn_normal_service(service: dict | None, service_id: ObjectId | None = None) -> bool:
    return bool(service and (service.get("name") or "").strip().lower() == "mtn normal")
...
def _is_at_provider_service(service: dict | None) -> bool:
    if not service:
        return False
    name = "".join(str(service.get("name") or "").strip().lower().split())
    return name in {"at-bigtime", "at-ishare", "atbigtime", "atishare"}
...
def manage_services():
    ...
    for s in services:
        s["_id_str"] = str(s["_id"])
        default_provider = "codecraft" if _is_at_provider_service(s) else None
        s["provider_label"] = _provider_label(s.get("provider") or default_provider)
...
def set_service_display(service_id):
    ...
    raw = request.form.get("display_on")
    payload = {}
    if raw is None and request.is_json:
        payload = request.get_json(silent=True) or {}
        raw = payload.get("display_on")
    display_on = _parse_bool_flag(raw)

    sync_campus_raw = payload.get("sync_campus") if request.is_json else None
    if sync_campus_raw is None:
        sync_campus_raw = request.form.get("sync_campus")
    sync_campus = _parse_bool_flag(sync_campus_raw)

    update_doc = {"display_on": display_on, "updated_at": datetime.utcnow()}
...
def set_service_provider(service_id):
    ...
    if provider not in SERVICE_PROVIDER_SET:
        return jsonify(
            {
                "success": False,
                "error": "provider must be one of: datakazina, codecraft, skplug, bundleportal",
            }
        ), 400
    ...
    is_at_provider_service = _is_at_provider_service(service)
    if not is_mtn_normal and not is_mtn_express and not is_telecel and not is_at_provider_service:
        return jsonify(
            {
                "success": False,
                "error": "Provider switch only allowed for MTN NORMAL, MTN EXPRESS, Telecel, AT - BigTime, or AT - iShare",
            }
        ), 400

    if is_at_provider_service and provider not in {"codecraft", "bundleportal"}:
        return jsonify(
            {
                "success": False,
                "error": "AT - BigTime and AT - iShare support CodeCraft or BundlePortal only",
            }
        ), 400
