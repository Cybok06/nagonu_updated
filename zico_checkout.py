...
from zico_activity_log import log_activity
from zico_order_display import build_order_display_items
from zico_order_job_queue import register_job_processor
from zico_tenant import resolve_admin_id_for_user_id
from zico_admin_paystack_ledger import evaluate_admin_wallet_low_balance
from zico_afa_settings_utils import load_afa_admin_base_price, load_afa_base_price
...
from zico_social_boosting_pricing import (
    ...
    try:
        ...
        if client_request_id:
            ...
            if existing:
                ...
                redirect_url = "/customer/boostings" if _is_boostings_only_result(existing_items) else f"/invoice/{existing.get('order_id')}"
                payload = {
                    "success": True,
                    ...
                    "main_admin_profit_total": _money(existing.get("main_admin_profit_total")),
                    "admin_profit_total": _money(existing.get("admin_profit_total")),
                    "store_profit_total": _money(existing.get("store_profit_total")),
        ...
        for idx, item in enumerate(cart, start=1):
            ...
            use_dataconnect = (use_dataconnect_express or use_dataconnect_mtn_normal) and not use_codecraft and not use_skplug
            use_datakazina = bool(
                api_allowed
                and (
            ...
            if not use_dataconnect and not use_datakazina and not use_codecraft and not use_skplug and not use_portal02:
                has_processing = True
                total_processing_amount += amt_total
        ...
        if total_to_charge_now <= 0:
            ...
            try:
                ...
                jlog("mtn_mashup_sms_error", order_id=order_id, error=str(exc))
            try:
                log_activity(
                    "order_placed",
            ...
            redirect_url = "/customer/boostings" if _is_boostings_only_result(results) else f"/invoice/{order_id}"
            return (
                jsonify(
                    {
                        "success": True,
        ...
        latest_order = orders_col.find_one({"order_id": order_id}, {"items": 1, "status": 1}) or {}
        response_items = latest_order.get("items") or results
        response_status = latest_order.get("status") or status
