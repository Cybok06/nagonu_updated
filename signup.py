...
def _generate_unique_agent_code() -> str:
    for _ in range(1000):
...
def _create_agent_code_for_user(user_id: ObjectId, now: datetime) -> None:
    if agent_codes_col.find_one({"user_id": user_id}, {"_id": 1}):
        return

    res = agent_codes_col.insert_one(
        {
            "user_id": user_id,
            "agent_code": _generate_unique_agent_code(),
            "status": "active",
            "created_at": now,
            "updated_at": now,
        }
...
def signup():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            _create_agent_code_for_user(user_id, now)
            ...
            try:
                if "user_id" in locals():
                    ...
                    agent_codes_col.delete_one({"user_id": user_id})
