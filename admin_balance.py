...
ARKESEL_API_KEY = "TGFhVVZvU3NOclJMZFJwWWJ5U2o"  # replace with env var in production
...
def _ensure_admin_balance_indexes():
    ...
    balances_col.create_index([("user_id", ASCENDING), ("updated_at", DESCENDING)], background=True)
...
def _balance_sort_key(item: dict):
    ...
    # most recent updated_at first; fallback to user _id desc
...
def view_balances():
    ...
    users_raw = list(
        users_col.find(
            user_query,
            {"first_name": 1, "last_name": 1, "phone": 1, "username": 1},
    ...
    if user_ids:
        for b in balances_col.find(
            {"$or": [{"user_id": {"$in": user_ids}}, {"user_id": {"$in": user_ids_str}}]},
            ...
            # Optional migration: normalize string user_id to ObjectId for this balance
    ...
    if missing_ids:
        ...
        balances_col.bulk_write(
            [
                UpdateOne(
                    {"user_id": uid},
                    {"$setOnInsert": {"user_id": uid, "amount": 0.0, "currency": "GHS", "created_at": now}},
                    upsert=True,
                )
    ...
    if _wants_json(request):
        ...
        json_payload["balances"] = [
            {
                "_id": b["_id"],
                "user": b["user"],
                "amount": b["amount"],
                "currency": b["currency"],
                "updated_at_str": b["updated_at_str"],
            }
            for b in balances
        ]
