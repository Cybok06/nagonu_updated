...
from checkout import _process_checkout_core, _phone_has_existing_order

def _require_customer():
    return bool(session.get("user_id")) and session.get("role") in {"customer"}

def _generate_api_key() -> str:
    return "NAGONU_" + secrets.token_urlsafe(24)
...
def _resolve_service_offer(payload: dict, user_oid: ObjectId, mode: str | None):
    ...
    if service_id:
        try:
            svc_doc = services_col.find_one({"_id": ObjectId(service_id)})
        except Exception:
            svc_doc = None
...
@agent_api_bp.route("/api/number_eligibility.php", methods=["GET", "POST"])
def agent_api_number_eligibility():
    user_oid, err_resp, err_code = _api_auth()
    if err_resp:
        return err_resp, err_code
    ...
    eligible = _phone_has_existing_order(phone)
    return jsonify(
        {
            "status": 200,
            "message": "Successful",
