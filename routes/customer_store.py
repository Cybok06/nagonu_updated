...
# ---------- helpers ----------
def _store_public_base_url() -> str:
    """Use the current origin locally, but retain the public store domain in production."""
    hostname = (urlsplit(request.host_url).hostname or "").strip().lower()
    if hostname in {"localhost", "127.0.0.1", "::1"}:
        return request.host_url.rstrip("/")
    return f"https://{STORE_PUBLIC_HOST}" if STORE_PUBLIC_HOST else request.host_url.rstrip("/")


def ensure_customer_store_indexes() -> None:
    """
    Keep the customer store withdraw flow responsive on live data.
    Safe to call repeatedly; MongoDB only creates missing indexes.
    """
    ...
    plans = [
        (
            ...
            [
                IndexModel(
                    [("owner_id", ASCENDING), ("slug", ASCENDING), ("status", ASCENDING)],
                    name="owner_slug_status_asc",
                ),
            ],
        ),
        (
        ...
        (
            store_accounts_col,
            [
                IndexModel([("store_slug", ASCENDING)], name="store_slug_asc"),
            ],
        ),
        (
            ...
            [
                IndexModel(
                    ...
                    name="owner_store_created_at_desc",
                ),
                IndexModel(
                    [
                        ("owner_id", ASCENDING),
                    ...
                    name="owner_store_method_status_amount_created_at_desc",
                ),
            ],
        ),
    ]

    for collection, models in plans:
        try:
            collection.create_indexes(models)
        except Exception as exc:
            print(f"[customer_store] index creation skipped for {collection.name}: {exc}")
...
def _owner_agent_code_doc(user_id: ObjectId) -> Dict[str, Any]:
...
def _owner_agent_code(user_id: ObjectId) -> str:
    return str((_owner_agent_code_doc(user_id) or {}).get("agent_code") or "").strip()


def _store_order_profit_expression() -> Dict[str, Any]:
    """Mongo expression for current and legacy store-order profit records."""
    return {
        "$sum": {
            "$map": {
                "input": {"$ifNull": ["$items", []]},
                                ...
                                "$max": [
                                    0,
                                    {
                                        "$subtract": [
                                            ...
                                            {"$toDouble": {"$ifNull": ["$$it.base_amount", 0]}},
                                        ]
                                    },
                                ]
                            },
...
def _profit_all_time(slug: str) -> float:
    order_profit_expr = _store_order_profit_expression()
    pipeline = [
...
def _process_wallet_withdrawal(
    owner_id: ObjectId,
    slug: str,
    amount: float,
    *,
    ...
    if updated.matched_count == 0:
        return {"success": False, "message": "Insufficient store profit balance"}
    ...
    request_doc = {
        ...
        "status": "paid",
        "note": note or "Wallet credited automatically",
        "created_at": now,
        "updated_at": now,
        "paid_at": now,
        "paid_by": "system",
    ...
    store_withdraw_requests.insert_one(request_doc)
...
def _maybe_auto_withdraw(owner_id: ObjectId, slug: str) -> Optional[Dict[str, Any]]:
    ...
    blocked_statuses = ["pending"] if method == "momo" else ["pending", "paid"]
    ...
    if method == "wallet":
        result = _process_wallet_withdrawal(
            owner_id,
            slug,
            amount,
            note="Wallet credited automatically via auto-withdraw",
            source="auto_withdraw",
        )
        if not result.get("success"):
            return None
        return {"id": result["id"], "reference": result["reference"], "status": "paid"}
...
def _gather_dashboard(slug: str) -> Dict[str, Any]:
    ...
    for o in recent_orders_cur:
        is_ussd_order = str(o.get("paid_from") or "").strip().lower() == "ussd" or bool(o.get("ussd"))
        ...
        if isinstance(items, list):
            for it in items:
                ...
                elif it.get("amount") is not None or it.get("base_amount") is not None:
                    found_store_profit = True
                    store_profit_total += max(
                        0.0,
                        round(_fmt_money(it.get("amount")) - _fmt_money(it.get("base_amount")), 2),
                    )
...
def customer_store_home():
    ...
    owner = users_col.find_one(
        {"_id": owner_id},
        {"full_name": 1, "name": 1, "username": 1, "email": 1, "stage_label": 1},
    )
    ...
    if not store_doc:
        ...
        return render_template(
            ...
            withdrawable=0.00,
            wallet_balance=_owner_wallet_balance(owner_id),
            agent_code=_owner_agent_code(owner_id),
            agent_code_doc=_owner_agent_code_doc(owner_id),
            ...
            store_public_base_url=_store_public_base_url(),
        )
...
@customer_store_bp.route("/customer/store/<slug>/agent-code/status", methods=["POST"])
def customer_store_agent_code_status(slug: str):
    if session.get("role") != "customer" or not session.get("user_id"):
        return redirect(url_for("login.login"))
    ...
    if new_status not in {"active", "inactive"}:
        return redirect(url_for("customer_store.customer_store_dashboard", slug=slug))
...
def api_customer_store_request_withdraw(slug: str):
    ...
    recent_pending = store_withdraw_requests.find_one({
        ...
        "store_slug": slug,
        "method": method,
        "status": {"$in": (["pending", "paid"] if method == "wallet" else ["pending"])},
        "amount": amount,
        "created_at": {"$gte": datetime.utcnow() - timedelta(minutes=2)}
    })
    if recent_pending:
        return jsonify({
            ...
            "id": str(recent_pending["_id"]),
            "reference": recent_pending.get("reference"),
            "status": recent_pending.get("status"),
        })
    ...
    if method == "wallet":
        result = _process_wallet_withdrawal(
            ...
            slug,
            amount,
            note="Wallet credited automatically",
            source="customer_request",
        )
        if not result.get("success"):
            return jsonify({"success": False, "message": result.get("message") or "Wallet withdrawal failed"}), 400
        return jsonify({
            "success": True,
            "id": result["id"],
...
# ---------- Payout settings ----------
@customer_store_bp.route("/customer/store/<slug>/withdraw", methods=["GET"])
def customer_store_withdraw_page(slug: str):
    if session.get("role") != "customer" or not session.get("user_id"):
        return redirect(url_for("login.login"))
