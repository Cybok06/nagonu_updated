...
wassce_checker_col = db["wassce_checker"]
store_checker_purchases_col = db["store_checker_purchases"]
...
RESULTS_CHECKER_SERVICE_ID = "results-checker"
RESULTS_CHECKER_TYPES = ("wassce", "bece")


def _results_checker_service() -> Dict[str, Any]:
    """Build the virtual store service from live admin checker inventory."""
    offers: List[Dict[str, Any]] = []
    available_total = 0
    for checker_type in RESULTS_CHECKER_TYPES:
        sample = wassce_checker_col.find_one(
            {"type": checker_type, "status": "not_sold"},
            sort=[("created_at", 1)],
        )
        available = wassce_checker_col.count_documents(
            {"type": checker_type, "status": "not_sold"}
        )
        available_total += available
        try:
            settings = checker_settings_col.find_one(
                {"_id": "checker_prices"}, {"prices": 1}
            ) or {}
            configured = float((settings.get("prices") or {}).get(checker_type) or 0)
            base = round(configured if configured > 0 else float((sample or {}).get("amount") or 0), 2)
        except Exception:
            base = 0.0
        offers.append({
            "amount": base,
            "value": {"checker_type": checker_type},
    ...
    return {
        ...
        "name": "Resuslt checker",
        "type": "RESULTS_CHECKER",
        "service_category": "results_checker",
        "unit": "checker",
        "offers": offers,
        "can_order": available_total > 0,
        "disabled_reason": "No result checkers are currently in stock." if available_total <= 0 else "",
        "image_url": "https://www.sirhans.com/images/checker.png",
    }


def _checker_type_from_item(item: Dict[str, Any]) -> str:
    value = item.get("value_obj") or item.get("value") or {}
    if isinstance(value, dict):
        checker_type = value.get("checker_type") or value.get("type")
    else:
        checker_type = value
    checker_type = str(checker_type or "").strip().lower()
    return checker_type if checker_type in RESULTS_CHECKER_TYPES else ""


def _ensure_checker_purchase_indexes() -> None:
    try:
        store_checker_purchases_col.create_index("token", unique=True, background=True)
        store_checker_purchases_col.create_index(
            [("store_slug", 1), ("paystack_reference", 1), ("line_index", 1)],
            unique=True,
            background=True,
        )
    except PyMongoError:
        # Index creation must never prevent the rest of the store from loading.
        pass


def _queue_store_checker_sms(purchase_id: Any) -> None:
    """Deliver a store checker once without delaying the checkout response."""
    if not purchase_id:
        return

    def _worker() -> None:
        now = datetime.utcnow()
        purchase = store_checker_purchases_col.find_one_and_update(
            {"_id": purchase_id, "sms_status": {"$nin": ["sending", "sent"]}},
            {
                "$set": {
                    "sms_status": "sending",
        ...
        msisdn = _normalize_sms_phone(str(purchase.get("phone") or ""))
        message = str(purchase.get("message") or "").strip()
        if not msisdn or not message:
            store_checker_purchases_col.update_one(
                {"_id": purchase_id, "sms_status": "sending"},
                {"$set": {
                    "sms_status": "invalid_phone" if not msisdn else "empty_message",
                    "sms_recipient": msisdn or "",
                    "sms_completed_at": datetime.utcnow(),
                    "updated_at": datetime.utcnow(),
                }},
            )
            return

        status = _send_arkesel_sms(msisdn, message)
        store_checker_purchases_col.update_one(
            {"_id": purchase_id, "sms_status": "sending"},
            {"$set": {
                "sms_status": status,
                "sms_recipient": msisdn,
                "sms_completed_at": datetime.utcnow(),
                "updated_at": datetime.utcnow(),
            }},
        )
        jlog(
            "store_results_checker_sms",
    ...
    try:
        ...
        jlog("store_results_checker_sms_spawn_error", purchase_id=str(purchase_id), error=str(exc))
...
try:
    from checkout import (  # type: ignore
        ...
        _resolve_network_id,
        _resolve_provider_network,
        _resolve_bundleportal_network,
        _resolve_codecraft_network_name,
        _resolve_package_size_gb,
        _resolve_skplug_gb_size,
        _resolve_datakazina_shared_bundle,
        _resolve_datakazina_mtn_network_id,
        _is_mtn_normal_service,
        _requires_existing_mtn_history,
        _phone_has_existing_order,
        _check_phone_history_requirement,
        _existing_mtn_history_enforced,
        _build_bundle_key,
        _has_processing_conflict_strict,
        _codecraft_get_packages_cached,
        _codecraft_submit_regular,
        _codecraft_submit_bigtime,
        _background_process_providers,
        _send_mashup_order_sms_async,
...
# Store keys (hardcoded): do NOT read from env
...
def _visible_services_query(extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    q: Dict[str, Any] = {"display_on": {"$ne": False}}
    if extra:
        q.update(extra)
    return q
...
def _owner_agent_code(user_id: Any) -> str:
    if not user_id:
        return ""
    user_id_str = str(user_id)
    doc = agent_codes_col.find_one(
        {
            "$or": [
                {"user_id": user_id},
                {"user_id": user_id_str},
        ...
        {"agent_code": 1, "status": 1, "updated_at": 1, "created_at": 1},
        sort=[("status", 1), ("updated_at", -1), ("created_at", -1)],
    )
    return str((doc or {}).get("agent_code") or "").strip()

def _service_state(svc: Dict[str, Any]) -> Dict[str, Any]:
    display_on = svc.get("display_on") is not False
    ...
    can_order = display_on and t in {"API", "OFF", "MANUAL"} and status == "OPEN" and availability == "AVAILABLE"
...
def _apply_store_pricing_to_service(
    ...
    for idx, of in enumerate(src_offers):
        ...
        if str(s.get("_id") or "") == RESULTS_CHECKER_SERVICE_ID:
            vt = str((of.get("value") or {}).get("checker_type") or of.get("value_text") or "").upper()
        else:
...
def _load_all_services_for_store_edit() -> List[Dict[str, Any]]:
    ...
    """
    fields = {"_id": 1, "name": 1, "offers": 1, "store_offers": 1, "unit": 1, "display_on": 1}
    raw = list(services_col.find(_visible_services_query(), fields))
    ...
    for r in raw:
        ...
        clean.append(s)
    clean.append(_results_checker_service())
    return clean

def _load_services_for_store_view(scope: str, ids: List[str]) -> List[Dict[str, Any]]:
    q: Dict[str, Any] = _visible_services_query()
    database_ids = [x for x in ids if str(x) != RESULTS_CHECKER_SERVICE_ID]
    if scope == "selected" and ids:
        try:
            q["_id"] = {"$in": [ObjectId(x) for x in database_ids if x]}
        except Exception:
            q["_id"] = {"$in": []}
    ...
    if RESULTS_CHECKER_SERVICE_ID in [str(x) for x in ids]:
        checker_service = _results_checker_service()
        checker_service["offers"] = [
            offer for offer in checker_service.get("offers") or []
            if float(offer.get("amount") or 0) > 0 and int(offer.get("available") or 0) > 0
        ]
        raw.append(checker_service)
    return raw
...
def _normalize_store_afa_config(store_doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if price < 0:
        price = 0.0
    admin_price = _admin_afa_settings_price(default=0.0)
    setup_invalid = bool(enabled and admin_price > 0 and price < admin_price)
    return {
        "visible": bool(enabled),
        "enabled": bool(enabled and price > 0 and not setup_invalid),
        "setup_invalid": setup_invalid,
        "disabled_reason": (
            "AFA Registration unavailable at this store due to wrong payment setup."
            if setup_invalid
            else ""
        ),
        "base_price": round(float(admin_price), 2),
        "price": round(float(price), 2),
        "title": "AFA Registration",
        "description": "Register your AFA quickly and securely.",
        "image_url": "",
    }


def _admin_afa_settings_price(default: float = 0.0) -> float:
    """
    Read the global admin AFA settings price.
    Returns `default` if missing/invalid.
    """
    try:
        doc = afa_settings_col.find_one({"_id": AFA_SETTINGS_ID}, {"price": 1})
...
def _sanitize_store_afa_product(cfg: Any) -> Dict[str, Any]:
    if not isinstance(cfg, dict):
        cfg = {}
    enabled = bool(cfg.get("enabled"))
    price = _to_float(cfg.get("price")) or 0.0
    base_price = _admin_afa_settings_price(default=0.0)
    if enabled and base_price > 0 and price < base_price:
        price = base_price
    if price < 0:
        price = 0.0
    return {
...
def _upsert_store_from_payload(owner_id: ObjectId, data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    doc = {
        ...
        "afa_product": _sanitize_store_afa_product(data.get("afa_product")),
...
def store_public_page(slug: str):
    ...
    owner_stage = (_lookup_user_any_status(store_doc.get("owner_id")) or {}).get("stage_label")
    afa_config = _normalize_store_afa_config(store_doc)
    offline_agent_code = _owner_agent_code(store_doc.get("owner_id"))
    ...
    return render_template(
        ...
        offline_agent_code=offline_agent_code,
        phone_history_verification_enabled=_existing_mtn_history_enforced(),
        announcement_popup=get_popup_announcement("store", session.get("user_id")),
        announcement_location="store",
...
def _verify_paystack(reference: str, profile: str = "primary") -> Tuple[bool, Dict[str, Any], str, Dict[str, Any]]:
...
def api_store_afa_register(slug: str):
    try:
        ...
        ok, verify_data, verify_message, _raw_verify = _verify_paystack(ps_ref, profile="primary")
        if not ok:
        ...
        admin_afa_price = _admin_afa_settings_price(default=0.0)
        store_profit_amount = max(0.0, round(expected_ghs - admin_afa_price, 2))
...
def api_store_paystack_public_key(slug: str):
    try:
        store_doc = stores_col.find_one({"slug": slug, "status": {"$ne": "deleted"}}, {"_id": 1})
        if not store_doc:
            return jsonify({"success": False, "message": "Store not found"}), 404

        day_key = _utc_day_key()
        key_used = "default"
        pk = PAYSTACK_PUBLIC_KEY if _is_pk(PAYSTACK_PUBLIC_KEY) else ""
...
def _server_reprice_store_cart(
    ...
    for item in cart:
        ...
        if str(service_id_raw or "") == RESULTS_CHECKER_SERVICE_ID:
            if RESULTS_CHECKER_SERVICE_ID not in [str(x) for x in (store_doc.get("service_ids") or [])]:
                revised.append({**item, "amount": 0.0, "base_amount": 0.0, "_checker_error": "Results Checker is not enabled for this store."})
                continue
            checker_type = _checker_type_from_item(item)
            virtual = _results_checker_service()
            priced_virtual = _apply_store_pricing_to_service(
                virtual, *_build_pricing_map(store_doc.get("pricing") or {})
            )
            offer = next(
                (o for o in priced_virtual.get("offers") or [] if _checker_type_from_item({"value_obj": o.get("value")}) == checker_type),
                None,
            )
            canonical = round(float((offer or {}).get("total") or 0), 2)
            canonical_base = round(float((offer or {}).get("amount") or 0), 2)
            error = ""
            if not checker_type:
                error = "Select a valid WASSCE or BECE checker."
            elif not offer or canonical_base <= 0:
                error = f"{checker_type.upper()} checker is out of stock."
            elif canonical < canonical_base:
                error = "Store checker price cannot be below the admin price."
            revised.append({
                **item,
                "kind": "results_checker",
                "value": checker_type.upper() if checker_type else item.get("value"),
                "value_obj": {"checker_type": checker_type},
                "amount": canonical,
                "base_amount": canonical_base,
                **({"_checker_error": error} if error else {}),
            })
            sys_total += canonical
            continue
        ...
        if canonical_base is None:
            canonical_base = _money(item.get("base_amount"))
...
def _finalize_store_profit_lines(results: List[Dict[str, Any]]) -> float:
    """Persist the store margin on every line and return the batch total.

    Store profit is always the customer selling price minus the price assigned
    to the store.  Doing this once after provider routing prevents individual
    provider branches from accidentally omitting the field.
    """
    total = 0.0
    for item in results:
        ...
        selling_price = 0.0 if skipped else _money(item.get("amount"))
        store_cost = 0.0 if skipped else _money(item.get("base_amount"))
        store_profit = max(0.0, round(selling_price - store_cost, 2))
        item["store_profit_amount"] = store_profit
        total += store_profit
...
def _store_checkout_handler(slug: str, body: Dict[str, Any]):
    try:
        ...
        if ps_ref:
            ...
            if prior:
                ...
                prior_orders = list(orders_col.find(
                    {"store_slug": slug, "batch_id": prior_batch_id}, {"order_id": 1}
                ).sort("batch_position", 1)) if prior.get("batch_id") else [prior]
                prior_order_ids = [doc.get("order_id") for doc in prior_orders if doc.get("order_id")]
                return jsonify(
                    {
                        "success": True,
                        ...
                        "result_url": prior.get("result_url"),
                        "redirect_url": prior.get("result_url") or url_for("checkout.invoice_batch_view", batch_id=prior_batch_id),
                        "idempotent": True,
                    }
                ), 200
        ...
        checker_requested: Dict[str, int] = {}
        for item in cart:
            if item.get("_checker_error"):
                return jsonify({"success": False, "message": item.get("_checker_error")}), 400
            if str(item.get("kind") or "").lower() == "results_checker":
                ct = _checker_type_from_item(item)
                checker_requested[ct] = checker_requested.get(ct, 0) + 1
        for checker_type, needed in checker_requested.items():
            in_stock = wassce_checker_col.count_documents(
                {"type": checker_type, "status": "not_sold"}
            )
            if in_stock < needed:
                return jsonify({
                    "success": False,
                    "message": f"Only {in_stock} {checker_type.upper()} checker(s) remain in stock.",
                }), 400
        ...
        for idx, item in enumerate(cart):
            ...
            phone_history_result = _check_phone_history_requirement(
                item.get("phone") or "",
                item.get("serviceName") or item.get("service_name"),
                item.get("service_network"),
                item.get("network") or item.get("network_group"),
                ...
                return jsonify(
                    {
                        ...
                        "serviceName": item.get("serviceName") or item.get("service_name") or "",
                    }
                ), 400
        ...
        if method == "admin_override":
            ...
            ok, verify_data, verify_message, raw_verify = _verify_paystack(ps_ref, profile=profile_used)
        ...
        if checker_requested:
            _ensure_checker_purchase_indexes()
        ...
        for idx, item in enumerate(cart, start=1):
            ...
            is_afa = str(item.get("kind") or "").strip().lower() == "afa_registration"
            is_results_checker = str(item.get("kind") or "").strip().lower() == "results_checker"

            if is_results_checker:
                checker_type = _checker_type_from_item(item)
                purchase_doc = store_checker_purchases_col.find_one({
                    "store_slug": slug, "paystack_reference": ps_ref, "line_index": idx,
                })
                checker = None
                if not purchase_doc:
                    checker = wassce_checker_col.find_one_and_update(
                        {"type": checker_type, "status": "not_sold"},
                        {"$set": {
                            "status": "sold", "sold_to_store": slug, "sold_channel": "store_page",
                            "delivery_phone": phone,
                            "sold_reference": ps_ref, "sold_at": datetime.utcnow(),
                        }},
                        sort=[("created_at", 1)],
                        return_document=ReturnDocument.AFTER,
                    )
                    if not checker:
                        return jsonify({
                            "success": False,
                            "message": f"{checker_type.upper()} checker became unavailable. Please contact support with payment reference {ps_ref}.",
                            "payment_reference": ps_ref,
                        }), 409

                    base_amount = round(float(checker.get("amount") or 0), 2)
                    if amt_total < base_amount:
                        wassce_checker_col.update_one(
                            {"_id": checker["_id"], "sold_reference": ps_ref},
                            {"$set": {"status": "not_sold"}, "$unset": {"sold_to_store": "", "sold_channel": "", "sold_reference": "", "sold_at": "", "delivery_phone": ""}},
                        )
                        return jsonify({"success": False, "message": "Checker price changed. Please refresh and try again."}), 409

                    purchase_doc = {
                        "token": uuid.uuid4().hex + uuid.uuid4().hex,
                        "store_slug": slug, "store_owner_id": store_doc.get("owner_id"),
                        "checker_id": checker.get("_id"), "checker_type": checker_type,
                        "message": checker.get("message") or "", "admin_price": base_amount,
                        "selling_price": amt_total, "profit_amount": round(amt_total - base_amount, 2),
                        "phone": phone, "paystack_reference": ps_ref, "line_index": idx,
                        "order_id": order_id, "sms_status": "pending", "sms_attempts": 0,
                        "created_at": datetime.utcnow(), "updated_at": datetime.utcnow(),
                    }
                    try:
                        store_checker_purchases_col.insert_one(purchase_doc)
                    except PyMongoError:
                        wassce_checker_col.update_one(
                            {"_id": checker["_id"], "sold_reference": ps_ref},
                            {"$set": {"status": "not_sold"}, "$unset": {"sold_to_store": "", "sold_channel": "", "sold_reference": "", "sold_at": "", "delivery_phone": ""}},
                ...
                _queue_store_checker_sms(purchase_doc.get("_id"))

                token = str(purchase_doc.get("token") or "")
                base_amount = round(float(purchase_doc.get("admin_price") or 0), 2)
                amt_total = round(float(purchase_doc.get("selling_price") or amt_total), 2)
                line_result_url = url_for("stores.store_checker_result", slug=slug, token=token)
                if not result_url:
                    result_url = line_result_url
                store_profit = round(amt_total - base_amount, 2)
                profit_amount_total += store_profit
                ...
                results.append({
                    ...
                    "amount": amt_total,
                    "profit_amount": store_profit,
                    "profit_percent_used": round((store_profit / base_amount) * 100, 2) if base_amount else 0.0,
                    "store_profit_amount": store_profit,
                    "value": checker_type.upper(),
                    "value_obj": {"checker_type": checker_type},
                    "serviceId": RESULTS_CHECKER_SERVICE_ID,
                    "serviceName": "Resuslt checker",
                    "service_type": "RESULTS_CHECKER",
                    "line_status": "completed",
                    "api_status": "not_applicable",
                    "sms_status": purchase_doc.get("sms_status") or "pending",
                    "result_url": line_result_url,
                    "api_response": {"note": "Result checker is available online and queued for SMS delivery."},
                })
                continue
            ...
            if not is_afa:
                ...
                if (
                    _existing_mtn_history_enforced()
                    and expected_prefixes
                    and detected_prefix
                    and detected_prefix not in expected_prefixes
                ):
            ...
            if not use_datakazina and not use_skplug and not use_codecraft and not use_bundleportal:
                ...
                if not api_allowed:
                    ...
                    note = (
                        "Not API eligible for this provider. Supported MTN providers here are DataKazina, SkPlug, and CodeCraft; queued for manual processing."
                    )
        ...
        # Payment has been verified before line processing.  Finalize every
        # provider result centrally so single and bulk orders use one formula.
        store_profit_total = _finalize_store_profit_lines(results)
        ...
        order_doc = {
            ...
            "platform_profit_amount_total": round(profit_amount_total, 2),
            "status": "processing",
            "paid_from": paid_from,
            "paystack_reference": ps_ref,
        ...
        for line_doc in order_docs:
            ...
            if str((line_doc["items"][0] or {}).get("kind") or "").lower() == "results_checker":
                store_checker_purchases_col.update_one(
                    {
                        "store_slug": slug,
                        "paystack_reference": ps_ref,
                        "line_index": line_doc.get("batch_position"),
                    },
                    {"$set": {"order_id": line_doc["order_id"], "batch_id": order_id}},
                )
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Order received and is processing. Order IDs: {', '.join(order_ids)}",
                ...
                "expected_ghs": expected_pay_ghs,
                "result_url": result_url,
                "redirect_url": result_url or url_for("checkout.invoice_batch_view", batch_id=order_id),
            }
        ), 200
...
@stores_bp.route("/store/<slug>/checker-result/<token>", methods=["GET"])
def store_checker_result(slug: str, token: str):
    if not re.fullmatch(r"[a-f0-9]{64}", token or ""):
        abort(404)
    purchase = store_checker_purchases_col.find_one(
        {"store_slug": slug, "token": token},
        {"message": 1, "checker_type": 1, "selling_price": 1, "created_at": 1, "store_slug": 1},
    )
    store_doc = stores_col.find_one(
        {"slug": slug, "status": {"$ne": "deleted"}},
        {"name": 1, "slug": 1, "logo_url": 1, "theme": 1},
    )
    if not purchase or not store_doc:
        abort(404)
    return render_template("store_checker_result.html", store=store_doc, purchase=purchase)
...
def api_store_complaint(slug: str):
    try:
        ...
        if not re.match(r"^0\d{9}$", phone):
            return jsonify({"success": False, "message": "Phone must be 10 digits in the format 0xxxxxxxxx (e.g. 0530393625)."}), 400
        if not paystack_ref:
            return jsonify({"success": False, "message": "Paystack reference is required"}), 400
        if not payment_date:
        ...
        if existing_pending:
            return jsonify(
                {
                    "success": False,
                    "message": "You submitted already. Please wait while we resolve it shortly.",
                }
            ), 409
