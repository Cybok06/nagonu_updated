...
def _send_dataverse_mtn_bundle(
    ...
    if not DATAVERSE_USERNAME or not DATAVERSE_PASSWORD:
        payload = {"status": "error", "message": "Dataverse credentials not configured", "http_status": 500}
        jlog("dataverse_config_error", phone=phone, order_reference=order_reference)
        debug_events.append({"when": datetime.utcnow(), "stage": "dataverse-config", "ok": False, "debug": payload})
    ...
    body = {"network": "mtn", "recipient": phone, "package_size": int(package_size_gb), "order_id": str(order_reference)}

    masked = phone[:3] + "***" + phone[-2:] if phone else ""
    jlog("dataverse_request_body", phone=masked, body={**body, "recipient": "***masked***"})

    try:
        resp = requests.post(DATAVERSE_PLACE_ORDER_URL, headers=headers, json=body, timeout=45)
        text = resp.text or ""
        try:
            data = resp.json()
        except Exception:
        ...
        debug = {"status": resp.status_code, "body_len": len(text), "body_snippet": text[:140].replace("\n", " ")}
        debug_events.append({"when": datetime.utcnow(), "stage": "dataverse-place-order", "ok": ok, "debug": debug})
        data["http_status"] = resp.status_code
        jlog("dataverse_call", phone=masked, order_reference=order_reference, ok=ok, status=resp.status_code, debug=debug)
        return ok, data
    except requests.RequestException as e:
        payload = {"status": "error", "message": str(e), "http_status": 599}
        debug_events.append({"when": datetime.utcnow(), "stage": "dataverse-error", "ok": False, "error": str(e)})
        jlog("dataverse_network_error", error=str(e), phone=masked, order_reference=order_reference)
        return False, payload
...
def store_checkout_paystack(slug: str):
    try:
        ...
        for idx, item in enumerate(cart, start=1):
            ...
            if svc_doc and is_api_enabled and phone and network_name == "mtn":
                package_size_gb = _derive_package_size_gb_from_item(svc_doc, value_obj, item.get("value"))
                if package_size_gb is not None and package_size_gb > 0:
                    trx_ref = f"{order_id}_{idx}"
                    ok2, api_payload = _send_dataverse_mtn_bundle(phone, package_size_gb, trx_ref, debug_events)
                    api_tag = "dataverse"
                    provider_reference = (api_payload.get("reference") or api_payload.get("order_id") or trx_ref)
                    api_status = "success" if ok2 else "processing"
                else:
                    api_status = "processing"
                    api_payload = {"note": "Could not derive Dataverse package_size from offer; queued for processing."}
            else:
                api_status = "processing"
                reason = []
